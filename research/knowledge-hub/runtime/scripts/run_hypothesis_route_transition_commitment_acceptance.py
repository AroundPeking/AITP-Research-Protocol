#!/usr/bin/env python
"""Isolated acceptance for the bounded hypothesis route transition-commitment surface."""
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-hypothesis-route-transition-commitment-acceptance-")).resolve()
    )
    ...
    topics = [
        ("demo-topic-no-commitment", "current_weak", "none"),
        ("demo-topic-commitment-waiting", "deferred_target", "waiting_resumption"),
        ("demo-topic-commitment-pending", "active_target_deferred", "pending_commitment"),
        ("demo-topic-commitment-committed", "current_target", "committed"),
    ]
    ...
    for topic_slug, route_mode, expected_status in topics:
        ...
        check(commitment["commitment_status"] == expected_status, f"Expected {topic_slug} to expose `{expected_status}`.")
        check(
            replay_bundle["route_transition_commitment"]["commitment_status"] == expected_status,
            f"Expected replay to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["current_position"]["route_transition_commitment_status"] == expected_status,
            f"Expected replay current position to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["conclusions"]["route_transition_commitment_status"] == expected_status,
            f"Expected replay conclusions to expose `{expected_status}` for {topic_slug}.",
        )
        check("## Route transition commitment" in runtime_protocol_text, f"Expected runtime protocol to include the route transition commitment for {topic_slug}.")
        check("## Route Transition Commitment" in replay_text, f"Expected replay markdown to include the route transition commitment for {topic_slug}.")

        if expected_status == "none":
            check(commitment["commitment_kind"] == "none", "Expected the no-commitment topic to keep kind none.")
        elif expected_status == "waiting_resumption":
            check(commitment["commitment_kind"] == "resumption_not_ready", "Expected the waiting topic to expose resumption_not_ready.")
        elif expected_status == "pending_commitment":
            check(commitment["commitment_kind"] == "active_route_not_yet_committed", "Expected the pending topic to expose active_route_not_yet_committed.")
            check(commitment["route_kind"] == "current_topic", "Expected the pending topic to keep current_topic route kind.")
            check("deferred_candidates.json" in (commitment.get("route_target_ref") or ""), "Expected the pending topic to keep a deferred buffer target ref visible.")
        else:
        ...
        check(
            ...
            f"Expected the bounded transition-commitment slice not to materialize a reactivated deferred candidate for {topic_slug}.",
        )
        ...
        checks[topic_slug] = {
            ...
            "route_kind": commitment["route_kind"],
