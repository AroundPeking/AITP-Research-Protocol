#!/usr/bin/env python
"""Isolated acceptance for the bounded hypothesis route transition-clearance surface."""
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-hypothesis-route-transition-clearance-acceptance-")).resolve()
    )
    ...
    topics = [
        ("demo-topic-no-clearance", True, False, "none"),
        ("demo-topic-clearance-awaiting", False, False, "awaiting_checkpoint"),
        ("demo-topic-clearance-blocked", False, True, "blocked_on_checkpoint"),
        ("demo-topic-clearance-cleared", False, True, "cleared"),
    ]
    ...
    for topic_slug, include_current_topic, checkpoint_trigger, expected_status in topics:
        ...
        check(clearance["clearance_status"] == expected_status, f"Expected {topic_slug} to expose `{expected_status}`.")
        check(
            replay_bundle["route_transition_clearance"]["clearance_status"] == expected_status,
            f"Expected replay to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["current_position"]["route_transition_clearance_status"] == expected_status,
            f"Expected replay current position to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["conclusions"]["route_transition_clearance_status"] == expected_status,
            f"Expected replay conclusions to expose `{expected_status}` for {topic_slug}.",
        )
        check("## Route transition clearance" in runtime_protocol_text, f"Expected runtime protocol to include the route transition clearance for {topic_slug}.")
        check("## Route Transition Clearance" in replay_text, f"Expected replay markdown to include the route transition clearance for {topic_slug}.")

        if expected_status == "none":
            check(clearance["clearance_kind"] == "none", "Expected the no-clearance topic to keep clearance kind none.")
        elif expected_status == "awaiting_checkpoint":
            check(clearance["clearance_kind"] == "checkpoint_not_opened", "Expected the awaiting topic to expose checkpoint_not_opened.")
        elif expected_status == "blocked_on_checkpoint":
            check(clearance["clearance_kind"] == "checkpoint_requested", "Expected the blocked topic to expose checkpoint_requested.")
            check(clearance["checkpoint_status"] == "requested", "Expected the blocked topic to keep checkpoint status requested.")
        else:
            check(clearance["clearance_kind"] == "checkpoint_answered", "Expected the cleared topic to expose checkpoint_answered.")
            check(clearance["checkpoint_status"] == "answered", "Expected the cleared topic to keep checkpoint status answered.")
            check("transition_history" in clearance["followthrough_ref"], "Expected the cleared topic to point back at the transition follow-through ref.")
        ...
        check(
            ...
            f"Expected the bounded transition-clearance slice not to materialize a reactivated deferred candidate for {topic_slug}.",
        )
        ...
        checks[topic_slug] = {
            ...
            "clearance_kind": clearance["clearance_kind"],
            "checkpoint_status": clearance["checkpoint_status"],
            "followthrough_ref": clearance["followthrough_ref"],
        }
