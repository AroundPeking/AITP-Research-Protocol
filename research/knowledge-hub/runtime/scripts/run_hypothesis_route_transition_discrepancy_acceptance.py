#!/usr/bin/env python
"""Isolated acceptance for the bounded hypothesis route transition-discrepancy surface."""
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-hypothesis-route-transition-discrepancy-acceptance-")).resolve()
    )
    ...
    topics = [
        ("demo-topic-no-discrepancy-pending", True, True, False, "none"),
        ("demo-topic-discrepancy-present", False, True, True, "present"),
        ("demo-topic-no-discrepancy-none", True, False, False, "none"),
    ]
    ...
    for topic_slug, include_current_topic, include_target_route, seed_recorded_receipt, expected_status in topics:
        ...
        check(discrepancy["discrepancy_status"] == expected_status, f"Expected {topic_slug} to expose `{expected_status}`.")
        check(
            replay_bundle["route_transition_discrepancy"]["discrepancy_status"] == expected_status,
            f"Expected replay to expose `{expected_status}` for {topic_slug}.",
        )
        check("## Route transition discrepancy" in runtime_protocol_text, f"Expected runtime protocol to include the route transition discrepancy for {topic_slug}.")
        check("## Route Transition Discrepancy" in replay_text, f"Expected replay markdown to include the route transition discrepancy for {topic_slug}.")

        if expected_status == "present":
            check(discrepancy["discrepancy_kind"] == "resolved_without_target_active", "Expected the discrepancy topic to expose the resolved-without-target-active kind.")
            check(discrepancy["severity"] == "attention", "Expected the discrepancy topic to be attention severity.")
        else:
            check(discrepancy["discrepancy_kind"] == "none", "Expected the non-discrepancy topic to expose kind none.")
        ...
        check(
            ...
            f"Expected the bounded transition-discrepancy slice not to materialize a reactivated deferred candidate for {topic_slug}.",
        )
