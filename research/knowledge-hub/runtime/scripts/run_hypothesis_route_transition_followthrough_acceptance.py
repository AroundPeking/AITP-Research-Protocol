#!/usr/bin/env python
"""Isolated acceptance for the bounded hypothesis route transition-followthrough surface."""
...
def mark_checkpoint_answered(*, package_root: Path, kernel_root: Path, repo_root: Path, topic_slug: str) -> None:
    ...
    checkpoint_path = kernel_root / "runtime" / "topics" / topic_slug / "operator_checkpoint.active.json"
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-hypothesis-route-transition-followthrough-acceptance-")).resolve()
    )
    ...
    topics = [
        ("demo-topic-no-followthrough", True, False, "none"),
        ("demo-topic-followthrough-awaiting", False, False, "held_by_clearance"),
        ("demo-topic-followthrough-blocked", False, True, "held_by_clearance"),
        ("demo-topic-followthrough-ready", False, True, "ready"),
    ]
    artifacts: dict[str, dict[str, str]] = {}
    checks: dict[str, dict[str, Any]] = {}
    for topic_slug, include_current_topic, checkpoint_trigger, expected_status in topics:
        seed_demo_runtime(
            kernel_root,
            topic_slug=topic_slug,
        ...
        if topic_slug == "demo-topic-followthrough-ready":
        ...
        check(followthrough["followthrough_status"] == expected_status, f"Expected {topic_slug} to expose `{expected_status}`.")
        check(
            replay_bundle["route_transition_followthrough"]["followthrough_status"] == expected_status,
            f"Expected replay to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["current_position"]["route_transition_followthrough_status"] == expected_status,
            f"Expected replay current position to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["conclusions"]["route_transition_followthrough_status"] == expected_status,
            f"Expected replay conclusions to expose `{expected_status}` for {topic_slug}.",
        )
        check("## Route transition follow-through" in runtime_protocol_text, f"Expected runtime protocol to include the route transition follow-through for {topic_slug}.")
        check("## Route Transition Followthrough" in replay_text, f"Expected replay markdown to include the route transition follow-through for {topic_slug}.")

        if expected_status == "none":
            check(followthrough["followthrough_kind"] == "none", "Expected the no-followthrough topic to keep kind none.")
        elif topic_slug == "demo-topic-followthrough-awaiting":
            check(followthrough["followthrough_kind"] == "awaiting_checkpoint", "Expected the awaiting topic to expose awaiting_checkpoint.")
        elif topic_slug == "demo-topic-followthrough-blocked":
            check(followthrough["followthrough_kind"] == "blocked_on_checkpoint", "Expected the blocked topic to expose blocked_on_checkpoint.")
            check(followthrough["checkpoint_status"] == "requested", "Expected the blocked topic to keep checkpoint status requested.")
        else:
            check(followthrough["followthrough_kind"] == "resume_from_followthrough_ref", "Expected the ready topic to expose resume_from_followthrough_ref.")
            check("transition_history" in followthrough["followthrough_ref"], "Expected the ready topic to point at the authoritative follow-through ref.")
        ...
        check(
            ...
            f"Expected the bounded transition-followthrough slice not to materialize a reactivated deferred candidate for {topic_slug}.",
        )
        ...
        checks[topic_slug] = {
            "followthrough_status": followthrough["followthrough_status"],
            "followthrough_kind": followthrough["followthrough_kind"],
            "checkpoint_status": followthrough["checkpoint_status"],
            "followthrough_ref": followthrough["followthrough_ref"],
        }
