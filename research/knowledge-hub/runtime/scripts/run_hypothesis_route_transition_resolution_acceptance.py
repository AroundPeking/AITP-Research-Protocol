#!/usr/bin/env python
"""Isolated acceptance for the bounded hypothesis route transition-resolution surface."""
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-hypothesis-route-transition-resolution-acceptance-")).resolve()
    )
    ...
    topics = [
        ("demo-topic-pending-resolution", True, True, False, "pending"),
        ("demo-topic-resolved-resolution", False, True, True, "resolved"),
        ("demo-topic-no-resolution", True, False, False, "none"),
    ]
    ...
    for topic_slug, include_current_topic, include_target_route, seed_recorded_receipt, expected_status in topics:
        ...
        check(route_resolution["resolution_status"] == expected_status, f"Expected {topic_slug} to expose `{expected_status}`.")
        check(
            replay_bundle["route_transition_resolution"]["resolution_status"] == expected_status,
            f"Expected replay to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["current_position"]["route_transition_resolution_status"] == expected_status,
            f"Expected replay current position to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["conclusions"]["route_transition_resolution_status"] == expected_status,
            f"Expected replay conclusions to expose `{expected_status}` for {topic_slug}.",
        )
        check("## Route transition resolution" in runtime_protocol_text, f"Expected runtime protocol to include the route transition resolution for {topic_slug}.")
        check("## Route Transition Resolution" in replay_text, f"Expected replay markdown to include the route transition resolution for {topic_slug}.")

        if expected_status == "pending":
            check(route_resolution["active_route_alignment"] == "source_active", "Expected the pending topic to keep the source route active.")
        elif expected_status == "resolved":
            check(route_resolution["receipt_status"] == "recorded", "Expected the resolved topic to retain a recorded receipt.")
            check(route_resolution["active_route_alignment"] == "no_local_active", "Expected the resolved topic to expose the current no-local-active alignment.")
        else:
            check(route_resolution["active_route_alignment"] == "source_active", "Expected the none topic to expose source_active alignment.")
        ...
        check(
            ...
            f"Expected the bounded transition-resolution slice not to materialize a reactivated deferred candidate for {topic_slug}.",
        )
        ...
        checks[topic_slug] = {
            "resolution_status": route_resolution["resolution_status"],
            "intent_status": route_resolution["intent_status"],
            "receipt_status": route_resolution["receipt_status"],
            "active_route_alignment": route_resolution["active_route_alignment"],
        }
