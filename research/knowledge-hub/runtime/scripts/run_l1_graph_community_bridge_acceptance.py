#!/usr/bin/env python
"""Isolated acceptance for community-level graph bridges feeding L1 -> L2 staging."""
...
def seed_runtime(runtime_root: Path) -> None:
    ...
    (runtime_root / "interaction_state.json").write_text(
        json.dumps(
            {
                "human_request": "Extract cross-source graph bridges from the current literature bundle.",
                "decision_surface": {
                    "selected_action_id": "action:demo-topic:community-bridge",
        ...
        json.dumps(
            {
                ...
                "summary": "Wait for bounded manual follow-up after community-bridge staging.",
            },
            ensure_ascii=True,
            separators=(",", ":"),
        )
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-l1-graph-community-bridge-acceptance-")).resolve()
    )
    ...
    write_concept_graph(
        ...
        community_label="Topological order cluster",
    ...
    check(
        ...
        "Expected graph analysis to detect one cross-source community bridge.",
    )
    check(
        graph_analysis["connections"][0]["kind"] == "shared_community_bridge",
        "Expected graph analysis to classify the bridge as shared_community_bridge.",
    )
    ...
    with patch.object(
        ...
        payload = service._execute_auto_actions(
            topic_slug="demo-topic",
            updated_by="graph-community-bridge-acceptance",
            max_auto_steps=1,
    ...
    check(
        ...
        "Expected literature_intake_stage to complete successfully from a shared community bridge.",
    )
    ...
    community_bridge_entries = [
        entry
        for entry in staged_entries
        if (entry.get("provenance") or {}).get("graph_analysis_kind") == "shared_community_bridge"
    ]
    check(
        len(community_bridge_entries) >= 2,
        "Expected community-bridge staging to materialize at least two shared-community downstream units.",
    )
    check(
        {entry["candidate_unit_type"] for entry in community_bridge_entries} == {"physical_picture", "workflow"},
        "Expected shared-community downstream units to materialize physical_picture + workflow.",
    )
    check(
        any("Topological order cluster" in str(entry.get("title") or "") for entry in community_bridge_entries),
        "Expected staged entry titles to carry the shared community label.",
    )
    check(
        "Topological order cluster" in graph_analysis_note.read_text(encoding="utf-8"),
        "Expected graph_analysis.md to render the shared community label.",
    )
