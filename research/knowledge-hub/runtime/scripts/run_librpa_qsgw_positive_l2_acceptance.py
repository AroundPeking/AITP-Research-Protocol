#!/usr/bin/env python
"""Acceptance for promoting the bounded LibRPA QSGW positive target into L2."""
...
REFERENCE_CANDIDATE_ID = "candidate:librpa-qsgw-deterministic-reduction-consistency-core"
TARGET_UNIT_ID = "claim:librpa-qsgw-deterministic-reduction-consistency-core"
CONSULT_QUERY = "LibRPA QSGW deterministic reduction thread consistency core"
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="lqpl2-")).resolve()
    )
    ...
    candidate_ledger_path = Path(contract_payload["candidate"]["ledger_path"])
    source_refs = list((contract_payload["target_contract"]["payload"] or {}).get("source_refs") or [])
    ...
    request_payload = service.request_promotion(
        ...
        notes=(
            "Promote the bounded LibRPA QSGW deterministic-reduction consistency core "
            "claim card into authoritative canonical L2."
        ),
    )
    approve_payload = service.approve_promotion(
        ...
        notes=(
            "Human-reviewed promotion: bounded deterministic-reduction consistency core only; "
            "broader convergence and whole-stack claims remain excluded."
        ),
    )
    promote_payload = service.promote_candidate(
        ...
        subdomain="librpa-qsgw",
        source_id=str(source_refs[0] if source_refs else ""),
        source_section="deterministic-reduction-consistency-core",
        source_section_title="LibRPA QSGW deterministic-reduction consistency core",
        notes=(
            "Promote only the bounded deterministic-reduction thread-consistency core on the "
        ...
        coverage_summary={
            "status": "pass",
            "summary": (
                "The bounded LibRPA QSGW deterministic-reduction consistency core has an explicit "
        ...
        consensus_summary={
            "status": "ready",
            "summary": (
                "The fresh first-principles target contract and trust gate agree on the bounded "
                "LibRPA QSGW positive claim."
            ),
        },
    )
    ...
    check(TARGET_UNIT_ID in consult_ids, "Expected consult_l2 to surface the promoted LibRPA QSGW positive claim.")
    check(
        any(
            str(row.get("knowledge_id") or "").strip() == TARGET_UNIT_ID
            for row in (compile_report_payload.get("payload") or {}).get("knowledge_rows", [])
        ),
        "Expected workspace knowledge report to include the promoted LibRPA QSGW positive claim.",
    )
