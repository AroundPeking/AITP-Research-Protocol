#!/usr/bin/env python
"""Isolated acceptance for the bounded hypothesis route transition-intent surface."""
...
def seed_demo_runtime(
    ...
    *,
    ...
    if include_current_topic:
        ...
        question = "What transition intent should remain visible while the local route stays active?"
        observables = ["Proposed route transition intent."]
        ...
        question = "What transition intent is waiting behind the current checkpoint?"
        ...
        question = "What transition intent is ready now that the local route is absent?"
    ...
    write_json(
        runtime_root / "topic_state.json",
        {
            ...
            "summary": "The active topic should surface the bounded route transition intent explicitly.",
            "pointers": {
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-hypothesis-route-transition-intent-acceptance-")).resolve()
    )
    ...
    topics = [
        ("demo-topic-proposed", True, False, "proposed"),
        ("demo-topic-ready", False, False, "ready"),
        ("demo-topic-checkpoint-held", False, True, "checkpoint_held"),
    ]
    ...
    for topic_slug, include_current_topic, request_checkpoint, expected_status in topics:
        ...
        check(route_intent["intent_status"] == expected_status, f"Expected {topic_slug} to expose `{expected_status}`.")
        check(
            replay_bundle["route_transition_intent"]["intent_status"] == expected_status,
            f"Expected replay to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["current_position"]["route_transition_intent_status"] == expected_status,
            f"Expected replay current position to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["conclusions"]["route_transition_intent_status"] == expected_status,
            f"Expected replay conclusions to expose `{expected_status}` for {topic_slug}.",
        )
        check("## Route transition intent" in runtime_protocol_text, f"Expected runtime protocol to include the route transition intent for {topic_slug}.")
        check("## Route Transition Intent" in replay_text, f"Expected replay markdown to include the route transition intent for {topic_slug}.")

        if expected_status == "proposed":
            check(route_intent["source_hypothesis_id"] == "hypothesis:weak-coupling", "Expected the proposed topic to retain the local source route.")
            check(route_intent["target_hypothesis_id"] == "hypothesis:symmetry-breaking", "Expected the proposed topic to point at the handoff target.")
            check("next_action_decision.md" in route_intent["source_route_ref"], "Expected the proposed topic to point at the local-route artifact.")
        elif expected_status == "ready":
            check(route_intent["source_hypothesis_id"] == "", "Expected the ready topic to have no active local source hypothesis.")
            check(route_intent["target_hypothesis_id"] == "hypothesis:symmetry-breaking", "Expected the ready topic to point at the handoff target.")
            check("deferred_candidates.json" in route_intent["target_route_ref"], "Expected the ready topic to point at the deferred-buffer target.")
        else:
            check(route_intent["gate_status"] == "checkpoint_required", "Expected the checkpoint-held topic to keep the gate status visible.")
            check("operator_checkpoint.active.md" in route_intent["gate_artifact_ref"], "Expected the checkpoint-held topic to point at the operator checkpoint note.")

        check(
            not read_jsonl(kernel_root / "runtime" / "topics" / topic_slug / "followup_reintegration.jsonl"),
        ...
        check(
            ...
            f"Expected the bounded transition-intent slice not to materialize a reactivated deferred candidate for {topic_slug}.",
        )
        ...
        checks[topic_slug] = {
            "intent_status": route_intent["intent_status"],
            "gate_status": route_intent["gate_status"],
            "source_hypothesis_id": route_intent["source_hypothesis_id"],
            "target_hypothesis_id": route_intent["target_hypothesis_id"],
        }
