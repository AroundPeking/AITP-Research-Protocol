#!/usr/bin/env python
"""Isolated acceptance for the bounded hypothesis route transition-repair surface."""
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-hypothesis-route-transition-repair-acceptance-")).resolve()
    )
    ...
    topics = [
        ("demo-topic-no-repair-pending", True, True, False, "none_required"),
        ("demo-topic-repair-needed", False, True, True, "recommended"),
        ("demo-topic-no-repair-none", True, False, False, "none_required"),
    ]
    ...
    for topic_slug, include_current_topic, include_target_route, seed_recorded_receipt, expected_status in topics:
        ...
        check(repair["repair_status"] == expected_status, f"Expected {topic_slug} to expose `{expected_status}`.")
        check(
            replay_bundle["route_transition_repair"]["repair_status"] == expected_status,
            f"Expected replay to expose `{expected_status}` for {topic_slug}.",
        )
        check("## Route transition repair" in runtime_protocol_text, f"Expected runtime protocol to include the route transition repair for {topic_slug}.")
        check("## Route Transition Repair" in replay_text, f"Expected replay markdown to include the route transition repair for {topic_slug}.")

        if expected_status == "recommended":
            check(repair["repair_kind"] == "redeclare_active_target_or_downgrade_resolution", "Expected the repair-needed topic to expose the expected repair kind.")
        else:
            check(repair["repair_kind"] == "none", "Expected the no-repair topic to expose repair kind none.")
        ...
        check(
            ...
            f"Expected the bounded transition-repair slice not to materialize a reactivated deferred candidate for {topic_slug}.",
        )
        ...
        checks[topic_slug] = {
            ...
            "discrepancy_status": repair["discrepancy_status"],
