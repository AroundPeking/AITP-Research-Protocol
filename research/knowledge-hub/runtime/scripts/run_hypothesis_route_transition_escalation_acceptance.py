#!/usr/bin/env python
"""Isolated acceptance for the bounded hypothesis route transition-escalation surface."""
...
def main() -> int:
    ...
    work_root = (
        ...
        if args.work_root
        else Path(tempfile.mkdtemp(prefix="aitp-hypothesis-route-transition-escalation-acceptance-")).resolve()
    )
    ...
    topics = [
        ("demo-topic-no-escalation", True, True, False, False, "none"),
        ("demo-topic-escalation-recommended", False, True, True, False, "checkpoint_recommended"),
        ("demo-topic-escalation-active", False, True, True, True, "checkpoint_active"),
    ]
    artifacts: dict[str, dict[str, str]] = {}
    checks: dict[str, dict[str, Any]] = {}
    for (
        topic_slug,
        include_current_topic,
        include_target_route,
        seed_recorded_receipt,
        checkpoint_trigger,
        expected_status,
    ) in topics:
        seed_demo_runtime(
        ...
        check(escalation["escalation_status"] == expected_status, f"Expected {topic_slug} to expose `{expected_status}`.")
        check(
            replay_bundle["route_transition_escalation"]["escalation_status"] == expected_status,
            f"Expected replay to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["current_position"]["route_transition_escalation_status"] == expected_status,
            f"Expected replay current position to expose `{expected_status}` for {topic_slug}.",
        )
        check(
            replay_bundle["conclusions"]["route_transition_escalation_status"] == expected_status,
            f"Expected replay conclusions to expose `{expected_status}` for {topic_slug}.",
        )
        check("## Route transition escalation" in runtime_protocol_text, f"Expected runtime protocol to include the route transition escalation for {topic_slug}.")
        check("## Route Transition Escalation" in replay_text, f"Expected replay markdown to include the route transition escalation for {topic_slug}.")

        if expected_status == "none":
            check(escalation["repair_status"] == "none_required", "Expected the no-escalation topic to keep repair status none_required.")
        elif expected_status == "checkpoint_recommended":
            check(escalation["repair_status"] == "recommended", "Expected the recommendation topic to keep repair status recommended.")
            check(escalation["checkpoint_status"] == "cancelled", "Expected the recommendation topic to have no active checkpoint yet.")
        else:
            check(escalation["checkpoint_status"] == "requested", "Expected the active escalation topic to hold an active checkpoint.")
        ...
        check(
            ...
            f"Expected the bounded transition-escalation slice not to materialize a reactivated deferred candidate for {topic_slug}.",
        )
        ...
        checks[topic_slug] = {
            ...
            "repair_status": escalation["repair_status"],
            "checkpoint_status": escalation["checkpoint_status"],
