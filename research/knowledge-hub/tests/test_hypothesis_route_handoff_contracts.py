...
class HypothesisRouteHandoffContractTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _seed_demo_topic(self) -> None:
        self._write_json(
            "runtime/topics/demo-topic/topic_state.json",
            {
                ...
                "summary": "Ready parked routes should surface a single bounded handoff candidate.",
            },
        )
        self._write_json(
            ...
            [
                {
                    ...
                    "summary": "Keep the weak-coupling route local while deciding which ready parked route should be the next bounded handoff candidate.",
                    "auto_runnable": False,
        ...
        self._write_json(
            "runtime/topics/demo-topic/research_question.contract.json",
            {
                ...
                "scope": ["Keep route handoff bounded to explicit route activation and re-entry artifacts."],
                ...
                "context_intake": ["Human request: surface one bounded route handoff candidate."],
                "source_basis_refs": ["paper:demo-source"],
                "interpretation_focus": ["Keep handoff candidates explicit without mutating runtime state."],
                "open_ambiguities": ["More than one parked route may be technically ready at the same time."],
    ...
    def test_status_and_replay_surface_route_handoff(self) -> None:
        ...
        route_handoff = status_payload["active_research_contract"]["route_handoff"]
        self.assertEqual(route_handoff["primary_handoff_candidate_id"], "hypothesis:symmetry-breaking")
        self.assertEqual(route_handoff["handoff_candidate_count"], 1)
        self.assertEqual(len(route_handoff["handoff_candidates"]), 1)
        self.assertEqual(route_handoff["handoff_candidates"][0]["handoff_status"], "handoff_candidate")
        self.assertEqual(route_handoff["handoff_candidates"][0]["reentry_status"], "reentry_ready")
        ...
        self.assertEqual(replay_payload["current_position"]["primary_handoff_candidate_id"], "hypothesis:symmetry-breaking")
        ...
        self.assertIn("## Route handoff", runtime_protocol_note)
        self.assertIn("handoff_status=`handoff_candidate`", runtime_protocol_note)
        self.assertIn("## Route Handoff", replay_note)
        self.assertIn("Keep parked", replay_note)
