...
class HypothesisRouteTransitionResumptionContractTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_no_resumption_when_no_followthrough_is_active(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-no-resumption",
            route_mode="current_weak",
            checkpoint_trigger=False,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-no-resumption")
        replay_payload = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-no-resumption")["payload"]

        resumption = status_payload["active_research_contract"]["route_transition_resumption"]
        self.assertEqual(resumption["resumption_status"], "none")
        self.assertEqual(resumption["resumption_kind"], "none")
        self.assertEqual(replay_payload["route_transition_resumption"]["resumption_status"], "none")
        self.assertEqual(replay_payload["current_position"]["route_transition_resumption_status"], "none")
        self.assertEqual(replay_payload["conclusions"]["route_transition_resumption_status"], "none")

    def test_resumption_waits_while_followthrough_is_held(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-resumption-waiting",
            route_mode="deferred_target",
            checkpoint_trigger=False,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-resumption-waiting")
        replay_payload = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-resumption-waiting")["payload"]

        resumption = status_payload["active_research_contract"]["route_transition_resumption"]
        self.assertEqual(resumption["resumption_status"], "waiting_followthrough")
        self.assertEqual(resumption["resumption_kind"], "followthrough_held")
        self.assertEqual(replay_payload["route_transition_resumption"]["resumption_status"], "waiting_followthrough")

    def test_resumption_pending_when_followthrough_is_ready_but_target_not_active(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-resumption-pending",
            route_mode="deferred_target",
            checkpoint_trigger=True,
        )
        self._mark_checkpoint_answered("demo-topic-resumption-pending")

        status_payload = self.service.topic_status(topic_slug="demo-topic-resumption-pending")
        replay_payload = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-resumption-pending")["payload"]

        resumption = status_payload["active_research_contract"]["route_transition_resumption"]
        self.assertEqual(resumption["resumption_status"], "pending")
        self.assertEqual(resumption["resumption_kind"], "ready_not_resumed")
        self.assertEqual(resumption["followthrough_status"], "ready")
        self.assertEqual(replay_payload["route_transition_resumption"]["resumption_status"], "pending")

    def test_resumption_visible_when_target_route_is_already_active(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-resumption-resumed",
            route_mode="current_target",
            checkpoint_trigger=False,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-resumption-resumed")
        replay_payload = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-resumption-resumed")["payload"]

        resumption = status_payload["active_research_contract"]["route_transition_resumption"]
        self.assertEqual(resumption["resumption_status"], "resumed")
        self.assertEqual(resumption["resumption_kind"], "target_route_active")
        self.assertEqual(resumption["active_route_alignment"], "target_active")
        self.assertEqual(resumption["active_local_hypothesis_id"], "hypothesis:symmetry-breaking")
        self.assertEqual(replay_payload["route_transition_resumption"]["resumption_status"], "resumed")
        self.assertEqual(replay_payload["current_position"]["route_transition_resumption_status"], "resumed")
        self.assertEqual(replay_payload["conclusions"]["route_transition_resumption_status"], "resumed")
