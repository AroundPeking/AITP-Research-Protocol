...
class HypothesisRouteTransitionDiscrepancyContractTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _seed_demo_topic(
        ...
        *,
        ...
        if include_current_topic and include_target_route:
            ...
            question = "Is there any bounded transition discrepancy while the source route still stays active?"
            observables = ["No route transition discrepancy is present."]
            ...
            question = "Is there any bounded transition discrepancy after receipt is recorded but the target route is not active?"
            observables = ["Present route transition discrepancy."]
            ...
            question = "Is any bounded transition discrepancy currently applicable?"
            observables = ["No route transition discrepancy is applicable."]
        ...
        self._write_json(
            f"runtime/topics/{topic_slug}/topic_state.json",
            {
                ...
                "summary": "The active topic should surface route transition discrepancy explicitly.",
        ...
        self._write_json(
            ...
            {
                "human_request": "Show the bounded route transition discrepancy.",
                "decision_surface": {
                    "selected_action_id": f"action:{topic_slug}:route-transition-discrepancy",
        ...
        self._write_jsonl(
            ...
            [
                {
                    "action_id": f"action:{topic_slug}:route-transition-discrepancy",
                    "status": "pending",
        ...
        self._write_json(
            f"runtime/topics/{topic_slug}/research_question.contract.json",
            {
                ...
                "research_mode": "formal_derivation",
                "question": question,
                "scope": ["Keep route transition discrepancy bounded to explicit resolution and upstream route artifacts."],
                ...
                "context_intake": ["Human request: surface the bounded route transition discrepancy."],
                "source_basis_refs": ["paper:demo-source"],
                "interpretation_focus": ["Keep transition discrepancy explicit without mutating runtime state."],
                "open_ambiguities": ["The bounded route handoff may or may not disagree with upstream route artifacts."],
                "competing_hypotheses": competing_hypotheses,
                "formalism_and_notation": ["Stay with the bounded demo notation."],
                "observables": observables,
                "target_claims": ["candidate:demo-claim"],
                "deliverables": ["Keep route transition discrepancy durable on the active topic surface."],
                "acceptance_tests": ["Runtime status and replay expose route transition discrepancy directly."],
                "forbidden_proxies": ["Do not infer route transition discrepancy from prose-only notes."],
                "uncertainty_markers": ["The bounded route handoff may still require explicit discrepancy surfacing."],
    ...
    def test_no_discrepancy_for_pending_transition(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-no-discrepancy-pending",
            include_current_topic=True,
            include_target_route=True,
            seed_recorded_receipt=False,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-no-discrepancy-pending")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-no-discrepancy-pending")
        replay_payload = replay_result["payload"]

        discrepancy = status_payload["active_research_contract"]["route_transition_discrepancy"]
        self.assertEqual(discrepancy["discrepancy_status"], "none")
        self.assertEqual(discrepancy["discrepancy_kind"], "none")
        self.assertEqual(discrepancy["severity"], "none")
        self.assertEqual(replay_payload["route_transition_discrepancy"]["discrepancy_status"], "none")

    def test_discrepancy_present_for_resolved_without_target_active(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-discrepancy-present",
            include_current_topic=False,
            include_target_route=True,
            seed_recorded_receipt=True,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-discrepancy-present")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-discrepancy-present")
        replay_payload = replay_result["payload"]

        discrepancy = status_payload["active_research_contract"]["route_transition_discrepancy"]
        self.assertEqual(discrepancy["discrepancy_status"], "present")
        self.assertEqual(discrepancy["discrepancy_kind"], "resolved_without_target_active")
        self.assertEqual(discrepancy["severity"], "attention")
        self.assertIn("transition_history", ",".join(discrepancy["discrepancy_artifact_refs"]))
        self.assertEqual(replay_payload["route_transition_discrepancy"]["discrepancy_status"], "present")

    def test_no_discrepancy_when_no_transition_applies(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-no-discrepancy-none",
            include_current_topic=True,
            include_target_route=False,
            seed_recorded_receipt=False,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-no-discrepancy-none")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-no-discrepancy-none")
        replay_payload = replay_result["payload"]
