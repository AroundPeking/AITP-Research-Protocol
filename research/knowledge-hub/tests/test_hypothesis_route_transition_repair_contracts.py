...
class HypothesisRouteTransitionRepairContractTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _seed_demo_topic(
        ...
        *,
        ...
        if include_current_topic and include_target_route:
            ...
            question = "What bounded repair plan is needed while no transition discrepancy is present?"
            observables = ["No route transition repair is required."]
            ...
            question = "What bounded repair plan is needed when resolved transition still does not align with the active route?"
            observables = ["Recommended route transition repair."]
            ...
            question = "Is any route transition repair currently applicable?"
            observables = ["No route transition repair is applicable."]
        ...
        self._write_json(
            f"runtime/topics/{topic_slug}/topic_state.json",
            {
                ...
                "summary": "The active topic should surface route transition repair explicitly.",
        ...
        self._write_json(
            ...
            {
                "human_request": "Show the bounded route transition repair.",
                "decision_surface": {
                    "selected_action_id": f"action:{topic_slug}:route-transition-repair",
        ...
        self._write_jsonl(
            ...
            [
                {
                    "action_id": f"action:{topic_slug}:route-transition-repair",
                    "status": "pending",
        ...
        self._write_json(
            f"runtime/topics/{topic_slug}/research_question.contract.json",
            {
                ...
                "research_mode": "formal_derivation",
                "question": question,
                "scope": ["Keep route transition repair bounded to explicit discrepancy and route artifacts."],
                ...
                "context_intake": ["Human request: surface the bounded route transition repair."],
                "source_basis_refs": ["paper:demo-source"],
                "interpretation_focus": ["Keep transition repair explicit without mutating runtime state."],
                "open_ambiguities": ["The bounded route handoff may or may not need explicit repair planning."],
                "competing_hypotheses": competing_hypotheses,
                "formalism_and_notation": ["Stay with the bounded demo notation."],
                "observables": observables,
                "target_claims": ["candidate:demo-claim"],
                "deliverables": ["Keep route transition repair durable on the active topic surface."],
                "acceptance_tests": ["Runtime status and replay expose route transition repair directly."],
                "forbidden_proxies": ["Do not infer route transition repair from prose-only notes."],
                "uncertainty_markers": ["The bounded route handoff may still require an explicit repair plan."],
    ...
    def test_no_repair_required_for_non_discrepant_pending_state(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-no-repair-pending",
            include_current_topic=True,
            include_target_route=True,
            seed_recorded_receipt=False,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-no-repair-pending")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-no-repair-pending")
        replay_payload = replay_result["payload"]

        repair = status_payload["active_research_contract"]["route_transition_repair"]
        self.assertEqual(repair["repair_status"], "none_required")
        self.assertEqual(repair["repair_kind"], "none")
        self.assertEqual(replay_payload["route_transition_repair"]["repair_status"], "none_required")

    def test_repair_recommended_for_present_discrepancy(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-repair-needed",
            include_current_topic=False,
            include_target_route=True,
            seed_recorded_receipt=True,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-repair-needed")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-repair-needed")
        replay_payload = replay_result["payload"]

        repair = status_payload["active_research_contract"]["route_transition_repair"]
        self.assertEqual(repair["repair_status"], "recommended")
        self.assertEqual(repair["repair_kind"], "redeclare_active_target_or_downgrade_resolution")
        self.assertIn("transition_history", ",".join(repair["repair_artifact_refs"]))
        self.assertEqual(replay_payload["route_transition_repair"]["repair_status"], "recommended")

    def test_no_repair_required_when_no_transition_applies(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-no-repair-none",
            include_current_topic=True,
            include_target_route=False,
            seed_recorded_receipt=False,
        )

        status_payload = self.service.topic_status(topic_slug="demo-topic-no-repair-none")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-no-repair-none")
        replay_payload = replay_result["payload"]
