...
class HypothesisRouteTransitionEscalationContractTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _seed_demo_topic(
        ...
        *,
        ...
        if include_current_topic and include_target_route:
            ...
            question = "When should route transition repair escalate into a checkpoint?"
            observables = ["No route transition escalation is required."]
            ...
            question = "When should the bounded route transition repair escalate into a human checkpoint?"
            observables = ["Route transition escalation becomes visible once repair remains active."]
            ...
            question = "Is any route transition escalation currently applicable?"
            observables = ["No route transition escalation is applicable."]
        ...
        self._write_json(
            f"runtime/topics/{topic_slug}/topic_state.json",
            {
                ...
                "summary": "The active topic should surface route transition escalation explicitly.",
        ...
        self._write_json(
            ...
            {
                "human_request": "Show the bounded route transition escalation.",
                "decision_surface": {
                    "selected_action_id": f"action:{topic_slug}:route-transition-escalation",
        ...
        self._write_jsonl(
            ...
            [
                {
                    "action_id": f"action:{topic_slug}:route-transition-escalation",
                    "status": "pending",
        ...
        self._write_json(
            f"runtime/topics/{topic_slug}/research_question.contract.json",
            {
                ...
                "research_mode": "formal_derivation",
                "question": question,
                "scope": ["Keep route transition escalation bounded to explicit repair and checkpoint artifacts."],
                ...
                "context_intake": ["Human request: surface the bounded route transition escalation."],
                "source_basis_refs": ["paper:demo-source"],
                "interpretation_focus": ["Keep transition escalation explicit without mutating runtime state."],
                "open_ambiguities": ["The bounded route handoff may still require a human checkpoint."],
                "competing_hypotheses": competing_hypotheses,
                "formalism_and_notation": ["Stay with the bounded demo notation."],
                "observables": observables,
                "target_claims": ["candidate:demo-claim"],
                "deliverables": ["Keep route transition escalation durable on the active topic surface."],
                "acceptance_tests": ["Runtime status and replay expose route transition escalation directly."],
                "forbidden_proxies": ["Do not infer route transition escalation from prose-only notes."],
                "uncertainty_markers": ["The bounded route handoff may still require an explicit checkpoint."],
    ...
    def test_no_escalation_when_repair_is_not_recommended(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-no-escalation",
        ...
        status_payload = self.service.topic_status(topic_slug="demo-topic-no-escalation")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-no-escalation")
        replay_payload = replay_result["payload"]

        escalation = status_payload["active_research_contract"]["route_transition_escalation"]
        self.assertEqual(escalation["escalation_status"], "none")
        self.assertEqual(escalation["repair_status"], "none_required")
        self.assertEqual(replay_payload["route_transition_escalation"]["escalation_status"], "none")
        self.assertEqual(replay_payload["current_position"]["route_transition_escalation_status"], "none")
        self.assertEqual(replay_payload["conclusions"]["route_transition_escalation_status"], "none")

    def test_escalation_recommended_when_repair_remains_uncheckpointed(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-escalation-recommended",
        ...
        status_payload = self.service.topic_status(topic_slug="demo-topic-escalation-recommended")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-escalation-recommended")
        replay_payload = replay_result["payload"]

        escalation = status_payload["active_research_contract"]["route_transition_escalation"]
        self.assertEqual(escalation["escalation_status"], "checkpoint_recommended")
        self.assertEqual(escalation["repair_status"], "recommended")
        self.assertEqual(escalation["checkpoint_status"], "cancelled")
        self.assertIn("transition_history", escalation["primary_repair_ref"])
        self.assertEqual(
            replay_payload["route_transition_escalation"]["escalation_status"],
            "checkpoint_recommended",
        )

    def test_escalation_active_when_checkpoint_is_already_requested(self) -> None:
        self._seed_demo_topic(
            topic_slug="demo-topic-escalation-active",
            include_current_topic=False,
            include_target_route=True,
            seed_recorded_receipt=True,
        ...
        status_payload = self.service.topic_status(topic_slug="demo-topic-escalation-active")
        replay_result = materialize_topic_replay_bundle(self.kernel_root, "demo-topic-escalation-active")
        replay_payload = replay_result["payload"]

        escalation = status_payload["active_research_contract"]["route_transition_escalation"]
        self.assertEqual(escalation["escalation_status"], "checkpoint_active")
        self.assertEqual(escalation["repair_status"], "recommended")
        self.assertEqual(escalation["checkpoint_status"], "requested")
        self.assertEqual(escalation["checkpoint_kind"], "contradiction_adjudication")
        self.assertIn("operator_checkpoint.active.md", escalation["checkpoint_ref"])
        self.assertEqual(
            replay_payload["route_transition_escalation"]["escalation_status"],
            "checkpoint_active",
        )
        self.assertEqual(
            replay_payload["current_position"]["route_transition_escalation_status"],
            "checkpoint_active",
        )
        self.assertEqual(
            replay_payload["conclusions"]["route_transition_escalation_status"],
            "checkpoint_active",
        )
