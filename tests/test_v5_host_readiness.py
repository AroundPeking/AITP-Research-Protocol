...
def test_priority_host_production_loop_uses_separate_lifecycle_probe_command(tmp_path):
    from brain.v5.host_readiness import audit_priority_host_production_loops
    from brain.v5.public_surfaces import require_valid_public_surface
    from brain.v5.trace import hook_trace_event_path
    from brain.v5.workspace import init_workspace
    ...
    payload = audit_priority_host_production_loops(
        ...
        check_installation=False,
        run_lifecycle_smoke=True,
        lifecycle_command=sys.executable,
        lifecycle_args=[str(script), str(hook_trace_event_path(ws))],
        timeout_seconds=10,
    )
    validated = require_valid_public_surface("runtime_host_production_loop_audit", payload)
    ...
    assert all(item["lifecycle_command"] == sys.executable for item in validated["items"])
    assert all(item["lifecycle_trace_delta_count"] == 1 for item in validated["items"])
...
def test_priority_host_production_loop_cli_mcp_accept_lifecycle_probe_command(tmp_path, capsys):
    from brain.v5.cli import main
    from brain.v5.mcp_tools import aitp_v5_audit_priority_host_production_loops
    from brain.v5.trace import hook_trace_event_path
    from brain.v5.workspace import init_workspace
    ...
    script.write_text(
        "\n".join(
            [
                ...
                "trace_path = Path(sys.argv[1])",
                "trace_path.parent.mkdir(parents=True, exist_ok=True)",
                "event = {'event_id': 'event-probe', 'session_id': 's1', 'topic_id': 'fqhe', 'event_type': 'tool_run_recorded', 'risk_level': 'guided', 'payload': {}, 'timestamp': '', 'kind': 'trace_event'}",
                "with trace_path.open('a', encoding='utf-8') as handle:",
                "    handle.write(json.dumps(event, sort_keys=True) + '\\n')",
                "print(json.dumps({'aitp': {'kind': 'hook_trace_event_record'}}))",
            ]
    ...
    mcp_payload = aitp_v5_audit_priority_host_production_loops(
        ...
        run_lifecycle_smoke=True,
        lifecycle_command=sys.executable,
        lifecycle_args=[str(script), str(trace_path)],
    )

    assert cli_payload["lifecycle_status_counts"] == {"lifecycle_observed": 3}
    assert mcp_payload["lifecycle_status_counts"] == {"lifecycle_observed": 3}
    assert all(item["lifecycle_command"] == sys.executable for item in cli_payload["items"])
