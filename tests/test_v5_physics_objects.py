...
def test_mandatory_reflection_ignores_empty_relation_failure_modes(tmp_path):
    from brain.v5.brief import build_execution_brief
    from brain.v5.physics_objects import record_object_relation, record_physics_object
    from brain.v5.workspace import bind_session, create_claim, create_topic, init_workspace
    ...
    assert brief["known_context"]["object_relations"][0]["failure_modes"] == []
    relation_questions = [
    ...
    assert all(item["intent_type"] != "object_relation_failure_mode_check" for item in brief["mandatory_reflection"])
    questions = "\n".join(item["question"] for item in brief["mandatory_reflection"])
    assert "recorded object-relation failure mode" not in questions
