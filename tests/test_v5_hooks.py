...
def test_claude_hook_script_pre_tool_blocks_summary_sourced_object_relation(tmp_path):
    ...
    claim = create_claim(
        ws,
        topic_id="fqhe",
        statement="Counting diagnoses the edge CFT sector.",
        evidence_profile="toy_numeric",
        confidence_state="hypothesis",
    ...
    hook_input = {
        ...
        "tool_input": {
            ...
            "statement": "The summary says counting diagnoses the edge CFT.",
            "source_kind": "findings",
            "source_ref": ".aitp/surfaces/session_summaries/s1/findings.md",
            "orientation_only": True,
        },
    ...
    assert result.returncode == 0
    assert payload["hookSpecificOutput"]["permissionDecision"] == "deny"
    assert payload["aitp"]["action"] == "record_object_relation"
    assert payload["aitp"]["block"] is True
    assert payload["aitp"]["policy_reasons"] == [
        {
            "policy_id": "no_summary_surface_as_truth_source",
...
def test_claude_hook_script_pre_tool_blocks_summary_sourced_sensemaking_report(tmp_path):
    import json
    import subprocess
    import sys
    from pathlib import Path
    ...
    hook_input = {
        ...
        "tool_input": {
            ...
            "summary": "The generated findings summary says the interpretation is settled.",
            "source_kind": "findings",
            "source_ref": ".aitp/surfaces/session_summaries/s1/findings.md",
            "orientation_only": True,
        },
    ...
    assert result.returncode == 0
    assert payload["hookSpecificOutput"]["permissionDecision"] == "deny"
    assert payload["aitp"]["action"] == "record_sensemaking_report"
    assert payload["aitp"]["block"] is True
    assert payload["aitp"]["policy_reasons"] == [
        {
            "policy_id": "no_summary_surface_as_truth_source",
...
def test_claude_hook_script_pre_tool_warns_code_method_validation_without_code_state(tmp_path):
    ...
    hook_input = {
        ...
        "tool_input": {
            "topic_id": "librpa-gw",
            "claim_id": claim.claim_id,
            "required_checks": ["benchmark_consistency"],
        },
    }
    ...
    assert payload["hookSpecificOutput"] == {
        "hookEventName": "PreToolUse",
        "permissionDecision": "allow",
        "permissionDecisionReason": "warn validate_claim; no_code_method_validation_without_code_state; required soon: record_code_state",
    }
    assert payload["aitp"]["action"] == "validate_claim"
    assert payload["aitp"]["mode"] == "warn"
    assert payload["aitp"]["required_actions"] == ["record_code_state"]


def test_claude_hook_script_pre_tool_denies_l2_promotion_without_evidence_refs(tmp_path):
    import json
    import subprocess
    import sys
    from pathlib import Path
...
def test_claude_hook_script_pre_tool_blocks_summary_sourced_apply_promotion_packet(tmp_path):
    import json
    import subprocess
    import sys
    from pathlib import Path
    ...
    hook_input = {
        ...
        "tool_input": {
            "packet_id": "packet-fqhe-counting",
    ...
    assert result.returncode == 0
    assert payload["hookSpecificOutput"]["permissionDecision"] == "deny"
    assert payload["aitp"]["action"] == "apply_promotion_packet"
    assert payload["aitp"]["block"] is True
    assert payload["aitp"]["policy_reasons"] == [
        {
            "policy_id": "no_summary_surface_as_truth_source",
