...
For validation, promotion-packet creation/application, and L2 promotion MCP
calls, Claude `PreToolUse` resolves the active typed claim plus evidence and
code-state refs, then reuses kernel policy before the tool runs. Other adapters
can call the same contract through
...
promotion-packet creation/application, when their requested source is only a
...
so adapter authors do not need to reconstruct record/validation/promotion-packet
sequencing from prose. Its `policy_reasons` list exposes machine-readable policy
...
the record-evidence/tool-run/execute-tool/subagent-ingestion,
validation-contract, promotion-packet creation/application, validation, and
promotion gate sequences, so runtimes can see that policy evaluation comes
