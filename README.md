...
For validation and L2 promotion MCP calls, Claude `PreToolUse` resolves the
active typed claim plus evidence/code-state refs and reuses kernel policy before
...
then reuses the same typed decision path.
