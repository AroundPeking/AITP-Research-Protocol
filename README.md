...
For validation, human-checkpoint request/decision, promotion-packet
creation/application, and L2 promotion MCP calls, Claude `PreToolUse` resolves
the active typed claim plus evidence and code-state refs, then reuses kernel
policy before the tool runs. Other adapters can call the same contract through
...
so adapter authors do not need to reconstruct record/checkpoint/validation/
promotion-packet sequencing from prose. Its `policy_reasons` list exposes machine-readable policy
