"""Kimi Code hook bridge for AITP v5."""
...
def _dispatch(args: argparse.Namespace, kimi_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if args.command == "post-tool":
        ...
        event = post_tool_use_trace_event(
            ...
            tool_name=_tool_name(kimi_payload),
            evidence_status=_evidence_status(kimi_payload),
        )
...
def _action_from_kimi_tool(payload: dict[str, Any]) -> str:
    tool_name = _tool_name(payload).lower()
    action = _action_from_aitp_mcp_tool(tool_name)
    if action:
        return action
    if tool_name == "bash":
        command = str(_tool_input(payload).get("command") or "").lower()
    ...
    *,
    ...
    if action == "change_claim_confidence" and not _has_trusted_apply_source(payload):
        return PolicyDecision(
            ...
            reasons=[
                PolicyReason(
                    policy_id="kimi_pre_tool_requires_trust_preflight",
    ...
    if action in {"destructive_action", "remote_execution", "expensive_compute"}:
        return PolicyDecision(
            ...
            reasons=[
                PolicyReason(
                    policy_id="kimi_pre_tool_requires_human_checkpoint",
                    message="High-risk Kimi tool use requires an explicit human checkpoint before execution.",
                    severity="block",
                )
            ],
...
def _has_trusted_apply_source(payload: dict[str, Any]) -> bool:
    tool_input = _tool_input(payload)
...
def _tool_input(payload: dict[str, Any]) -> dict[str, Any]:
    value = payload.get("tool_input") or payload.get("input") or _nested(payload, "tool", "input")
...
def _tool_name(payload: dict[str, Any]) -> str:
    return (
        ...
        or _string_value(_nested(payload, "tool", "name"))
...
def _kimi_pre_tool_output(decision, *, action: str, policy_decision: PolicyDecision) -> dict[str, Any]:
...
def _evidence_status(payload: dict[str, Any]) -> str:
    explicit = _string_value(payload.get("evidence_status")) or _string_value(payload.get("status"))
    if explicit:
        return explicit
    response = payload.get("tool_response") or payload.get("response") or payload.get("result")
    if isinstance(response, dict):
        if response.get("exit_code") == 0 or response.get("status") in {"success", "completed", "passed"}:
            return "completed"
...
def _kimi_continue(extra: dict[str, Any]) -> dict[str, Any]:
    return {
        "continue": True,
        "suppressOutput": True,
        **extra,
