...
- Codex can now merge AITP v5 lifecycle hooks into an existing Codex
  `hooks.json` file through
...
- The shared CLI/MCP/runtime pre-tool policy now hard-blocks rigorous or
  ...
  current `recipe_id`.
- Validation result records now link a completed `tool_run_id` back to a
...
- `aitp-v5 memory request-failure-mode-review --claim <claim-id>` /
  ...
  `--failure-mode-review-checkpoint` / `failure_mode_review_checkpoint_id` plus
  a passed `--failure-mode-review-result` /
...
- `brain/v5/trust_audit.py`: read-only claim confidence support audit payloads
  derived from typed records.
- `brain/v5/trust_audit_contracts.py`: public contract for
  `claim_trust_audit`.
