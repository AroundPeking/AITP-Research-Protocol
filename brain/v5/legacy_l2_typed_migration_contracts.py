...
def _validate_review_group(payload: Any, path: str, result: ContractResult) -> None:
    ...
    if isinstance(payload.get("sample_work_items"), list):
        for index, item in enumerate(payload["sample_work_items"]):
            _validate_sample_work_item(item, f"{path}.sample_work_items[{index}]", result)


def _validate_sample_work_item(payload: Any, path: str, result: ContractResult) -> None:
    _require_mapping(payload, path, result)
    if not isinstance(payload, dict):
        return
    for key in ("work_item_id", "work_item_kind", "legacy_id", "role", "status", "source_path", "migration_action"):
        _require_nonempty_str(payload, key, path, result)
    _require_bool_value(payload.get("can_update_claim_trust"), False, f"{path}.can_update_claim_trust", result)
